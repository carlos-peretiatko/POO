...
public class Computador {
    ...
    public Computador(String nSerie, String modeloProcessador, double clockProcessador, double armazenamento,
        double memoria,
    ...
    public String fazerOverclock(int incrementoMHz) {
        if (!overclock)
            return "Este processador não permite overclock.";
        double incrementoGHz = incrementoMHz / 1000.0;
        double maxClock = clockProcessador * 1.10;
        if (clockProcessador + incrementoGHz > maxClock)
            return "Limite de 10% do clock original excedido.";
        double novoConsumo = consumoEnergia + ((incrementoMHz / 100.0) * 75);
        if (novoConsumo > potenciaFonte * 0.95)
            return "Consumo excede 95% da potência da fonte.";
...
// Fazer método construtor, atributos privados, getters e setters, método
// toString, tratamento de
...
// construtor, mas
// atribuido true no construtor caso o modelo do processador termine em 'K' ou
// 'KF' ou 'X' e false
// caso não termine.
// Faça:
// a) Um método que faça overclock no processador.
// O método deve ter como parâmetro o incremento em MHz que será feito de
// overclock.
...
// Note que o processador somente aceitará overclock caso a variável booleana de
// overclock for
// true.
// Caso não seja possível fazer o overclock, por qualquer uma das limitações,
// informe por que
// não foi possível.
...
// prédio.
// Eles querem saber antes qual o impacto do total de computadores nos gastos de
// energia
// elétrica.
// Informe qual a potência mínima em Watts que o circuito elétrico deve ter para
// acomodar todas
// as salas.
// A potência mínima é calculada a partir do consumo total e adicionado 10% por
// questões de
// segurança.
// d) Assumindo que os computadores vão funcionar por uma média de 12 horas por
// dia, informe
